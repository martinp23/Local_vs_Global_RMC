...
% for global vs local swap later conn = conn_loc with conn = conn_glob
% setup of the script
show_connectivity = 1;
show_heads = 1;
show_numbering = 0;
show_hydrogens = 1;
print_figure = 0;
special_rotation = 1; % this to be turn on for porphyrin nanorings.
...
conn_file_glob = [xyz_path,xyz_file(1:end-4),'_connectivity_global.csv'];
conn_file_loc = [xyz_path,xyz_file(1:end-4),'_connectivity_local.csv'];
conn_file_glob_in = [xyz_path,xyz_file(1:end-4),'_connectivity_global_in.csv'];
conn_file_glob_out = [xyz_path,xyz_file(1:end-4),'_connectivity_global_out.csv'];
conn_file_loc_in = [xyz_path,xyz_file(1:end-4),'_connectivity_local_in.csv'];
conn_file_loc_out = [xyz_path,xyz_file(1:end-4),'_connectivity_local_out.csv'];



% conn_file = conn_file_glob_out;
% conn_file = conn_file_glob_in;
% conn_file = conn_file_loc_in;
conn_file = conn_file_glob;

% conn_file = conn_file_loc_out;
%conn_file = conn_file_glob;
% conn_file = conn_file_loc; % change this for other model

conn = csvread(conn_file);
...
if run_RCGF
    ...
    x = RCM_anal(conn,idx, geomTXT, geomNO, separation, special_rotation,conn_file);
